...
def build_qa_datasets(train_path, val_path, tok, max_len=384, doc_stride=128):
    ...
    for split, path in [("train", train_path), ("validation", val_path)]:
        ...
        def prep_train(examples):
            ...
            impossible = examples.get("is_impossible", [False]*len(sample_map))
            ...
            for i, off in enumerate(offsets):
                ...
                if impossible[sidx] or len(ans.get("text", [])) == 0:
                    start_positions.append(0); end_positions.append(0)
                    tokenized["offset_mapping"][i] = [(0,0)]*len(off)
                    continue
                ...
                if tok_start > ctx_end or tok_end > ctx_end:
                    ...
                    start_positions.append(tok_start); end_positions.append(tok_end)
...
def main():
    ...
    ap.add_argument("--model_name", default="deepset/roberta-base-squad2")
